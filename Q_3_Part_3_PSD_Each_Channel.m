...
psd_results_f = cell(size(f, 1), size(frequency_bands, 2));
psd_results_nf = cell(size(nf, 1), size(frequency_bands, 2));
...
for i = 1:size(f, 1) % Loop over channels
    ...
    for j = 1:size(frequency_bands, 2) % Loop over frequency bands
        ...
        [pxx_nf, nf_axis] = pmtm(squeeze(nf(i, :, :)), 3.5, size(nf, 2), fs, 'ConfidenceLevel', 0.95);
        ...
        xlabel('Frequency (Hz)');
        ylabel('PSD(dB/Hz)');
        title(sprintf('Channel %d - Face Condition - %s Band', i, frequency_bands{j}));
        ...
        xlabel('Frequency (Hz)');
        ylabel('PSD (dB/Hz)');
        title(sprintf('Channel %d - Non-Face Condition - %s Band', i, frequency_bands{j}));
    end
end
